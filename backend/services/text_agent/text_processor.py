...
from shared.agents.base.base_processor import BaseProcessor
...
class TextProcessor(BaseProcessor):
    ...
    async def process_batch_classification(self, job_data: dict) -> Dict[str, Any]:
        ...
        print(f"   🤖 Using child AI model: {child_ai_model} (for each text classification)")
        print(f"   📡 Mother AI model used for instructions: {mother_ai_model}")
    ...
    async def classify_with_pure_ai(self, content: str, available_labels: List[str], 
        ...
        # Create a comprehensive classification prompt with confidence scoring
...
{{"label": "chosen_label", "confidence": 0.95, "reasoning": "explanation", "alternative_labels": ["label2", "label3"], "confidence_factors": ["factor1", "factor2"]}}

Choose from: {', '.join(available_labels)}

CONFIDENCE SCORING GUIDELINES:
- 0.9-1.0: Very confident, clear indicators present
- 0.7-0.89: Confident, some indicators present
- 0.5-0.69: Moderately confident, mixed signals
- 0.3-0.49: Low confidence, weak indicators
- 0.1-0.29: Very low confidence, unclear classification

Include confidence_factors explaining what influenced your confidence level."""
        ...
        try:
            ...
            try:
                ...
                if parsed_response:
                    ...
                    alternative_labels = parsed_response.get("alternative_labels", [])
                    confidence_factors = parsed_response.get("confidence_factors", [])
                    ...
                    if predicted_label in available_labels:
                        ...
                        return {
                            ...
                            "reasoning": reasoning,
                            "alternative_labels": alternative_labels[:3],  # Limit to top 3
                            "confidence_factors": confidence_factors[:5],  # Limit to top 5
                            "model_used": model_used,
                            "method": "pure_ai_classification_with_confidence"
                        }
                    else:
