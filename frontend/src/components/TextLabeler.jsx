...
const TextProcessor = () => {
  ...
  const submitFileJob = async () => {
    if (!selectedFile || !availableLabels.trim() || !motherAiModel || !childAiModel) {
      alert('Please select a file, provide available labels, and choose both Mother AI and Child AI models')
      return
    }
    ...
    try {
      ...
      formData.append('labels', availableLabels)
      formData.append('instructions', instructions)
      formData.append('mother_ai_model', motherAiModel)
      formData.append('child_ai_model', childAiModel)

      const response = await fetch('http://localhost:8000/api/v1/submit-batch-job', {
        method: 'POST',
        body: formData,
      })
      ...
      const pollStatus = async () => {
        try {
          const statusResponse = await fetch(`http://localhost:8000/api/v1/jobs/${newJobId}`)
          const statusData = await statusResponse.json()
          
          setJobStatus(statusData.status)
          setProgress(statusData.progress)  // Backend already sends percentage
          console.log('Progress update:', statusData.progress, '%')
          ...
          if (statusData.status === 'completed') {
            ...
            setIsProcessing(false)
            alert('Job failed: ' + (statusData.error || 'Unknown error'))
          } else {
      ...
      console.error('Error submitting file job:', error)
      setIsProcessing(false)
      setJobStatus('failed')
  ...
  const resetForm = () => {
    ...
    setAvailableLabels('')
    setInstructions('Analyze each text and assign the most appropriate label from the provided labels, also make sure that you analyze all the text properly.')
    setMotherAiModel('')
    setChildAiModel('')
    setJobId(null)
    setJobStatus(null)
          ...
          <CardContent className="space-y-4">
            {(isProcessing || progress > 0) && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">Debug: progress = {progress}</p>
              </div>
            )}
            ...
            {jobStatus === 'completed' && result && (
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-600">
                  Your text classification has been completed. Download the labeled results below.
                </p>
                <Button 
